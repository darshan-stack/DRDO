...
if ROS_AVAILABLE:
    class FFEMNode(Node):
        def __init__(self):
            ...
            self.declare_parameter('input_topic','/points'); self.declare_parameter('map_frame','map'); self.declare_parameter('use_tf',True); self.declare_parameter('queue_depth',5); self.declare_parameter('max_points_per_frame',150000); self.declare_parameter('enable_rerun',False); self.declare_parameter('recording','outputs/ros_ffem.rrd')
            self.declare_parameter('map_topic','~/map/elevation'); self.declare_parameter('moving_topic','~/map/moving_points'); self.declare_parameter('metrics_topic','~/metrics'); self.declare_parameter('events_topic','~/refinement_events')
            ...
            self.pipeline=FFEMPipeline(cfg); self.frame=0; self.map_frame=str(self.get_parameter('map_frame').value); self.max_points=int(self.get_parameter('max_points_per_frame').value); self.use_tf=bool(self.get_parameter('use_tf').value); self.tf_buffer=Buffer(); self.tf_listener=TransformListener(self.tf_buffer,self)
            ...
            self.map_pub=self.create_publisher(PointCloud2,str(self.get_parameter('map_topic').value),5); self.moving_pub=self.create_publisher(PointCloud2,str(self.get_parameter('moving_topic').value),5); self.metrics_pub=self.create_publisher(Float32MultiArray,str(self.get_parameter('metrics_topic').value),5); self.events_pub=self.create_publisher(String,str(self.get_parameter('events_topic').value),5)
            ...
            self.get_logger().info(f'FFEM integrated node listening on {self.get_parameter("input_topic").value}; TF={self.use_tf}; Rerun={self.rerun_enabled}')
        ...
        def _publish(self,result,stamp):
            mp,_,_=self.pipeline.mapping.arrays(); self.map_pub.publish(encode_pointcloud2(mp,frame_id=self.map_frame,stamp=stamp)); moving=result['points'][result['moving']]; self.moving_pub.publish(encode_pointcloud2(moving,frame_id=self.map_frame,stamp=stamp)); s=result['stats']; m=Float32MultiArray(); m.data=[float(s[k]) for k in ('total_ms','map_ms','active_cells','topology_changes','points','moving_points')]; self.metrics_pub.publish(m)
