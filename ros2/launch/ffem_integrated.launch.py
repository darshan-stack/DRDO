...
def generate_launch_description():
    input_topic=LaunchConfiguration('input_topic'); map_frame=LaunchConfiguration('map_frame')
    return LaunchDescription([
        DeclareLaunchArgument('input_topic', default_value='/carla/ego_vehicle/lidar'),
        DeclareLaunchArgument('map_frame', default_value='map'),
        Node(package='ffem_lidar_mapping', executable='ffem_node', name='ffem_mapper', output='screen', parameters=[{'input_topic': input_topic, 'map_frame': map_frame, 'use_tf': True, 'enable_rerun': False}]),
