...
ROOT=Path(__file__).resolve().parents[1]; STATE=ROOT/'outputs'/'dashboard_state.json'; FINEST=0.05
class State:
    def __init__(self):
        self.lock=threading.Lock(); self.data={
            "frame":0,"points":0,"moving":0,"active_cells":0,"uniform_cells":0,
            "memory_reduction_pct":0.0,"total_ms":0.0,"map_ms":0.0,"topology_changes":0,
            "tracks":0,"classes":[0]*7,"semantic":[],"semantic_labels":[],"adaptive":[],
            "adaptive_levels":[],"moving_points":[],"updated":0.0}
    def write(self):
        with self.lock:
            STATE.parent.mkdir(parents=True,exist_ok=True); tmp=STATE.with_suffix('.tmp')
            tmp.write_text(json.dumps(self.data,separators=(',',':'))); tmp.replace(STATE)
class DashboardNode(Node):
    def __init__(self,state):
        super().__init__('ffem_dashboard'); self.s=state
        self.last_sem=np.empty((0,3),np.float32); self.last_map=np.empty((0,3),np.float32)
        self.last_move=np.empty((0,3),np.float32); self.last_labels=np.empty((0,),np.int32)
        self.last_levels=np.empty((0,),np.int32); self.metrics=[0.0]*7; self.seq=0
    ...
    def sem_cb(self,msg):
        ...
        if self.last_labels is None:self.last_labels=np.zeros(len(self.last_sem),np.int32)
    def map_cb(self,msg):
        self.last_map,self.last_levels=self.pts(msg)
        if self.last_levels is None:self.last_levels=np.zeros(len(self.last_map),np.int32)
        self.publish_state()
    ...
    def publish_state(self):
        ...
        with self.s.lock:
            self.seq+=1
            self.s.data.update({
                'frame':int(self.seq),'points':int(self.metrics[4]),'moving':int(self.metrics[5]),
                'active_cells':adaptive,'uniform_cells':uniform,'memory_reduction_pct':float(reduction),
                'total_ms':float(self.metrics[0]),'map_ms':float(self.metrics[1]),
                'topology_changes':int(self.metrics[3]),'tracks':int(self.metrics[6]),
                'classes':np.bincount(self.last_labels,minlength=7).tolist() if len(self.last_labels) else [0]*7,
                'semantic':self.down(self.last_sem).tolist(),'semantic_labels':self.down(self.last_labels).tolist(),
                'adaptive':self.down(self.last_map).tolist(),'adaptive_levels':self.down(self.last_levels).tolist(),
...
def main():
    ...
    threading.Thread(target=web.serve_forever,daemon=True).start(); print(f'Dashboard: http://{args.host}:{args.port}/')
    try:rclpy.spin(node)
